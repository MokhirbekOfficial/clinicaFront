...
function Home() {
  ...
  const getId = (evt) => {
    ...
    fetch("http://192.168.1.9:4000/services", requestOptions)
      .then((data) => data.json())
      .then((m) => {
        setServices(m);
      });
  ...
  const tekeOrder = (evt) => {
    ...
    fetch("http://192.168.1.9:4000/add/order", requestOptions)
      .then((data) => data.json())
      .then((m) => {
        if (m == "ok") {
  ...
  const getOrders = (evt) => {
    ...
    fetch("http://192.168.1.9:4000/user/getorders", requestOptions)
      .then((data) => data.json())
      .then((m) => {
        setMyOrders(m)
      });
  ...
  return (
    ...
    <section className="myOrders" ref={myOrder2}>
      <div className="navbar">
          ...
          <span ref={homeActive2} onClick={homeActive}>Home</span>
        </div>
        ...
        {myOrders &&
            ...
            <li className="myOrdersListItem" key={item.order_id}>
                ...
                <h2><p>Patient name:</p>{item.user_name}</h2>
                <div className="serviceT">
                <span className="serviceTitle">{item.service_title}</span>
                <span className="turnTime">{normalizeDate(item.order_time)}</span>
                </div>
                <h3><p>Doctor name:</p>{item.doctor_name}</h3>
                <div className="turnValue">
                <span className="doctorTel"><p>Doctor tel:</p>{item.doctor_tel}</span>
